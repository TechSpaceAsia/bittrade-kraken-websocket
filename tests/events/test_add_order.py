...
from bittrade_kraken_websocket.events.add_order import AddOrderRequest, create_order_lifecycle, order_related_messages_only
...
def test_order_related_only():
    ...
    results = scheduler.start(lambda: messages.pipe(order_related_messages_only("real-order-id")))
...
def test_add_order_success():
    ...
    caller = add_order_factory_full(sender, socket_messages, scheduler.create_cold_observable(on_next(1000, None)))
    result = scheduler.start(lambda: caller(options, 5))
    expected = [
        on_next(300, Order(
            order_id="OXW22X-FYBXP-JQDBJT",
            status="submitted"
        )),
        on_next(400, Order(
            order_id="OXW22X-FYBXP-JQDBJT",
...
def test_add_order_failed():
    ...
    assert result.messages == [on_error(250, RequestResponseError("Unsupported field: 'refid' for the given msg type: add order"))]
    assert socket_messages.subscriptions == [
        from_to(200, 250),
        from_to(200, 250),
    ]
...
def test_add_order_timeout_initial():
    ...
    caller = add_order_factory_full(sender, socket_messages,
                                    scheduler.create_cold_observable(on_next(300, None)))  # 200+300 < 600
...
def test_add_order_timeout_details():
    ...
    caller = add_order_factory_full(sender, socket_messages,
                                    scheduler.create_cold_observable(on_next(350, None)))  # 200+350 < 600
    result = scheduler.start(lambda: caller(options, 50))
    assert result.messages[0] == on_next(300, Order(order_id="OXW22X-FYBXP-JQDBJT", status="submitted"))
    m = result.messages[1]
    assert m.time == 550
    assert m.value.kind == 'E'
    assert type(m.value.exception) == TimeoutError
